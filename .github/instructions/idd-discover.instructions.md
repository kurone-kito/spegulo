...
3. Apply A3's readiness bullets to the target — no configured
   ...
   `spegulo-blocked-by` marker, both resolved the
...
NOT contain a `spegulo-roadmap-id` marker (not itself
a roadmap) or a `spegulo-blocked-by` marker, AND
...
`spegulo-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`spegulo-roadmap-id` marker. Retro-label a legacy
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `spegulo-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `spegulo-roadmap-id` or
  `spegulo-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `spegulo-roadmap-id` marker when checking
  `spegulo-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- spegulo-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- spegulo-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- spegulo-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- spegulo-effort: S|M|L -->` footer
...
- **Roadmap identity** (`spegulo-roadmap-id`): placed in
...
- **Sequential dependency** (`spegulo-blocked-by`): placed
...
**Do not use `spegulo-blocked-by` to group sub-tasks under
