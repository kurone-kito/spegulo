...
| --- | --- | --- | --- |
| Local containers | NVIDIA Container Toolkit configures the container runtime (`nvidia-ctk runtime configure --runtime=docker`) so `docker run --gpus all` or Podman's `--gpus`/CDI device selection exposes the GPU [^toolkit] [^podman-run] | Docker Desktop's GPU support is WSL2-backend-only; the NVIDIA driver installs on the **Windows host only**, never inside the WSL2 distro; only `--gpus all` is supported, not per-GPU index selection, on Docker 19.03 [^wsl-cuda] [^docker-gpu] | Installing a Linux display driver inside WSL2 instead of relying on the Windows host driver; using Docker's Hyper-V backend instead of the WSL2 backend; a pre-R495 driver or pre-Pascal GPU [^wsl-cuda] |
| Local VM | Linux: VFIO PCI passthrough, needs IOMMU (Intel VT-d / AMD-Vi) enabled and the GPU bound to `vfio-pci` instead of its normal driver [^ubuntu-kvm-gpu]. Windows: Hyper-V Discrete Device Assignment (DDA) or GPU Partitioning (GPU-P) [^dda-deploy] [^gpu-p] | WSL2 itself is not a general-purpose VM the operator provisions, but running it doesn't preclude also using Hyper-V on the same Windows machine -- the binding constraint is the Windows Server / enterprise-GPU requirement in the next column, not WSL2's presence | Linux: IOMMU disabled in BIOS, a device stuck in a shared IOMMU group, or a missing driver blocklist [^ubuntu-kvm-gpu]. Windows: **both DDA and GPU-P require a Windows Server host**; per Microsoft's own requirements, DDA "requires server class hardware" [^dda-plan], and GPU-P's supported GPU list is enterprise cards (NVIDIA A2/A10/A16/A40/L2/L4/L40/L40S/RTX Pro 6000 Blackwell Server Edition, AMD Radeon PRO V710) [^gpu-p] -- neither is available on a Windows 11 client host with a consumer GPU, which is the maintainer's actual machine class |
...
| Bare local install | Native OS GPU driver install; no passthrough boundary to cross | On the maintainer's actual host, "bare" most naturally means installing directly inside the WSL2 distro without a container, not bypassing WSL2 to bare Windows. GPU access still depends on the same Windows-host driver WSL2 requires for any GPU workload -- there is no separate WSL2 driver [^wsl-cuda] -- reached through whatever local-inference backend's own GPU library linkage is installed, not the Docker Container Toolkit path this document otherwise documents | A missing or incompatible native driver only |
...
[^podman-run]: Podman, ["podman-run(1)"](https://docs.podman.io/en/latest/markdown/podman-run.1.html), Podman documentation -- documents `--gpus` as functional GPU support (`all` or a vendor identifier), not a no-op, while noting a specific vendor among multiple present devices needs `--device` with a CDI name.
